...
function SignatureRenderer({ signature }: { signature: DocNode }) {
...
function CategorySection({ title, items, icon: Icon, color }: { title: string, items: DocNode[], icon: React.ComponentType<{ className?: string }>, color: string }) {
...
export default function DocPageContent({ slug }: { slug: string[] }) {
        ...
        <div className="space-y-10 animate-in fade-in duration-500 pb-20">
            ...
            {signatures.length > 0 && (
                <section>
                    ...
                    <div className="space-y-6">
                        {signatures.map(sig => <SignatureRenderer key={sig.id} signature={sig} />)}
                    </div>
                </section>
            )}
            ...
            {constructors.length > 0 && (
                <section>
                    ...
                    {constructors.map(c =>
                        c.signatures?.map(sig => <SignatureRenderer key={sig.id} signature={sig} />)
                    )}
                </section>
            )}
            ...
            {methods.length > 0 && (
                <section>
                    ...
                    <div className="space-y-6">
                        {methods.map(m =>
                            m.signatures?.map(sig => <SignatureRenderer key={sig.id} signature={sig} />)
                        )}
                    </div>
                </section>
            )}
